class EmbedMediumTranslationObserver < ActiveRecord::Observer
  ...
  def after_create(record)
    ...
    record.progress_story_id = record.embed_medium.section.story_id
  ...
  def after_commit(record)
    Rails.logger.debug "@@@@@@@@@@ embed_medium trans after commit, story id #{record.progress_story_id; }; is_progress_increment = #{record.is_progress_increment}"
